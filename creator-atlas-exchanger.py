...
def get_plist_images(plist_path: str, image_dict: dict[str, ImageRef]):
    ...
    if plist is not None:
        ...
        for img_name in image_names:
            ...
            image_dict[img_name] = ImageRef(
                atlas_name,
                img_name,
                ref["uuid"],
                ref["rawTextureUuid"],
                atlas_uuid,
                atlas_rawTextureUuid,
            )
...
class ImageRefReplace:
    ...
    def __str__(self):
        return (
            self._image_from.atlas()
            + "["
            + self._image_from.name()
            + "] -> "
            + self._image_to.atlas()
            + "["
            + self._image_to.name()
            + "]"
        )
...
def contains_src_uuid(
    images_from: dict[str, ImageRef], images_to: dict[str, ImageRef], line: str
) -> list[ImageRefReplace]:
    replaces = []
    for k in images_from:
        ...
        if len(o.atlasUuid()) > 0 and str(line).find(o.atlasUuid()) != -1:
            replaces.append(ImageRefReplace(o, images_to[o.name()]))
        if (
            len(o.atlasRawTextureUuid()) > 0
            and str(line).find(o.atlasRawTextureUuid()) != -1
        ):
            replaces.append(ImageRefReplace(o, images_to[o.name()]))
    return replaces


def change_image_sprite_frame_refer(
    images_from: dict[str, ImageRef], images_to: dict[str, ImageRef], project_path: str
) -> int:
    ...
    for root, _, files in os.walk(assets):
        for fn in files:
            if not (
                fn.lower().endswith(".anim")
                or fn.lower().endswith(".prefab")
                or fn.lower().endswith(".fire")
            ):
                continue
            ...
            for i in range(0, len(content)):
                ...
                log.head("will replace sprite frames\n" + sf + "\nin " + file_path)
                change_count += 1

                for r in replaces:
                    if len(r.from_uuid()) > 0:
                        content[i] = content[i].replace(r.from_uuid(), r.to_uuid())
    ...
    for i in range(0, len(from_keys)):
        ...
        if key not in to_images:
            ...
            try:
                ...
                print("from_images No key: '%s'" % key)
